...
func changeFont(line string) string {
	...
	for _, c := range line {
		...
		paragraph += fmt.Sprintf("<span style='font-size:%dpx;color:%s;background-color:%s;font-style:%s;text-decoration:%s;'>%s</span>", rand.Intn(32)+16, color, bgColor, randItalic(), randTextDecor(), string(c))
	}
	paragraph += "</p>"
	return paragraph
}
